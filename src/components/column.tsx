...
export function Column({ id, title, index }: ColumnDef & { index: number }) {
    ...
    <Draggable draggableId={id} index={index}>
      {(provided, snapshot) => (
        <section
          ...
          className={`flex max-h-full w-81 shrink-0 flex-col rounded-lane bg-lane transition-shadow motion-reduce:transition-none ${snapshot.isDragging ? "shadow-card-hover ring-1 ring-accent/40" : ""}`}
        >
          {/* The header doubles as the column drag handle. Interactive controls
              inside it (the rename input, Save/Cancel, the Options button) never
              start a drag — the library blocks drags that begin on interactive
              elements — and the options wrapper stops mousedown propagation so
              its full-screen, non-interactive backdrop can't begin one either. */}
          <div
            {...provided.dragHandleProps}
            className={`group flex shrink-0 items-center gap-2 px-4 pt-3.5 pb-2.5 ${editing ? "" : "cursor-grab active:cursor-grabbing"}`}
          >
            {editing ? (
              <>
          ...
          {/* space-y (margins), not gap: the dnd engine measures margin boxes when
              ...
              emptied lane droppable now that columns hug their content. type="card"
              isolates this from the board-column reorder droppable. */}
          <Droppable droppableId={id} type="card">
            {(dropProvided, dropSnapshot) => (
              <ul
                ref={dropProvided.innerRef}
                {...dropProvided.droppableProps}
                className={`scrollbar-subtle flex min-h-10 flex-1 flex-col space-y-[9px] overflow-y-auto px-2.5 pt-0.5 pb-1.5 transition-colors motion-reduce:transition-none ${dropSnapshot.isDraggingOver ? "bg-accent/5" : ""}`}
              >
